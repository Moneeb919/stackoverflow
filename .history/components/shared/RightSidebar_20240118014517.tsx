  ...
  return (
    <div className="custom-scrollbar text-dark300_light900 fixed right-0 top-0 flex h-screen w-fit flex-col overflow-y-auto pt-36">
      ...
      <div className="text-dark300_light900 mb-0">
        <p className="m-8 text-2xl font-bold">Popular Tags</p>
          ...
          {dummyRightSideTag.map((item) => {
            return (
              <div key={item.tag}>
                <Link
                  href="/"
                  className="m-8 flex cursor-pointer justify-between"
                >
                  <div className="rounded-md bg-gray-100 px-4 py-2 text-xs text-gray-700">
                    {item.tag}
                  </div>
                  <div>{item.num}</div>
                </Link>
